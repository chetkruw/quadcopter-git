/*Support code for main file. Contains implementations for all header file methods
...
/*! Attitude control: Implement basic attitude angle testing using a simple while loop.
	Drone should take pitch angle and move for a set time before breaking. then it should angle
	the negative of the angle and return to original position.
!*/
bool
moveByAngleVertThrust(Vehicle *vehicle, float roll,
                     ...
                     float thrust)
{
  ...
  //! Main closed-loop angle time control
  while (elapsedTimeInMs < timeoutInMilSec)
  {
    //! @note 18 is the flag value of this mode
    uint8_t ctrl_flag =
      (vehicle->control->VERTICAL_THRUST | vehicle->control->HORIZONTAL_ANGLE | vehicle->control->YAW_ANGLE | vehicle->control->HORIZONTAL_BODY);
    Control::CtrlData data = {ctrl_flag, roll, pitch, thrust, yaw};
  ...
  int breakTime = 0;
  while (breakTime<1000)
  {
    float broll = -35*roll/abs(roll);
    float bpitch = -35*pitch/abs(pitch);
    float byaw = -yaw;
    uint8_t ctrl_flag_break = (vehicle->control->VERTICAL_THRUST | vehicle->control->HORIZONTAL_ANGLE | vehicle->control->YAW_ANGLE | vehicle->control->HORIZONTAL_BODY);
    Control::CtrlData data = {ctrl_flag_break, broll, bpitch, 100, byaw};
    vehicle->control->flightCtrl(data);

    usleep(cycleTimeInMs * 1000);
    breakTime += cycleTimeInMs;
  }
...
/*! Monitored Landing (Blocking API call). Return status as well as ack.
    This version of Landing makes sure your aircraft actually took off
    and only returns when Landing is complete.
    Use unless you want to do other stuff during Landing - this will block
    the main thread.
!*/
bool
